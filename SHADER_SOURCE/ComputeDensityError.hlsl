...
void CS_MAIN(uint3 DispatchThreadID : SV_DispatchThreadID)
{
	...
	float densityPrediction = pi.density;
	//From Fluid
	for (int x = -1; x <= 1; x++) {
		for (int y = -1; y <= 1; y++) {
			for (int z = -1; z <= 1; z++) {
				...
				while (pjIndex < particlesNum) {
					...
					if (dist < 2 * radius && dist > 1e-5f) {
						densityPrediction += mass * deltaTime * dot(velocityDiff, cubic_spline_kernel_gradient(diff));
					}
					pjIndex++;
				}
			}
		...
		for (int y = -1; y <= 1; y++) {
			for (int z = -1; z <= 1; z++) {
				...
				while (pjIndex < particlesNum) {
					...
					if (dist < 2 * radius && dist > 1e-5f) {
						densityPrediction += mass * deltaTime * dot(pi.velocity, cubic_spline_kernel_gradient(diff));
					}
					pjIndex++;
				}
			}
	...
	stiffness[piIndex] = densityPrediction - restDensity;
}